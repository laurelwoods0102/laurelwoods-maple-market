...
def define():
  ...
  flags.DEFINE_string('train_log_dir', './train',  #/tmp/attention_ocr/train
                      'Directory where to write event logs.')

  flags.DEFINE_string('dataset_name', 'laurelwoods_maple_dataset', #fsns
  ...
  flags.DEFINE_float('learning_rate', 0.04,  #0.004
                     'learning rate')

  flags.DEFINE_string('optimizer', 'adam',  #momentum
                      'the optimizer to use')

  flags.DEFINE_float('momentum', 0.9,
                     'momentum value for the momentum optimizer if used')

  flags.DEFINE_bool('use_augment_input', False, #True
  ...
  flags.DEFINE_integer('num_lstm_units', 256,  #256
